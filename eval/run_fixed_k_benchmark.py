...
RESULTS_DIR = Path("eval/results/fixed")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run fixed-k retrieval benchmarks.")
...
def main() -> None:
    ...
    for k in args.k:
        debug_path = results_dir / f"fixed_k{k}_debug.jsonl"
        metrics_path = results_dir / f"fixed_k{k}_metrics.json"
        debug_rows = []

        print(f"Running fixed k={k}")
        for idx, row in enumerate(dataset_rows, start=1):
            print(f"[k={k} {idx}/{len(dataset_rows)}] {row['question']}")
            result = analyze_one(
                row["question"],
                pipe,
                tokenizer,
